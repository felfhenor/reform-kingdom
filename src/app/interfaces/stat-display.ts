...
// Drives `RowLabeledValuesComponent`: display order, friendly labels, icons,
// and which keys are percentages (vs a flat number like `agroValue`).
export type StatDisplayDimension<K extends string = string> = {
  ...
  // every dimension shipped so far except `CombatantCombatStats.agroValue`.
