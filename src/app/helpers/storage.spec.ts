...
describe('Storage Helper Functions', () => {
  ...
  describe('getStorageMaterials', () => {
    it('returns owned materials sorted alphabetically by name', () => {
      setAllContentById(
        new Map([
      ...
      vi.mocked(gamestate).mockReturnValue({
        materials: {
          [tatteredHide.id]: { quantity: 2 },
          [goldCoin.id]: { quantity: 5 },
        },
      } as unknown as GameState);

      expect(getStorageMaterials()).toEqual([
        { item: goldCoin, quantity: 5 },
        { item: tatteredHide, quantity: 2 },
      ]);
    });
    ...
    it('excludes materials with zero quantity', () => {
      ...
      vi.mocked(gamestate).mockReturnValue({
        materials: { [goldCoin.id]: { quantity: 0 } },
      } as unknown as GameState);
    ...
    it('excludes materials with no matching content entry', () => {
      setAllContentById(new Map());
      vi.mocked(gamestate).mockReturnValue({
        materials: { [goldCoin.id]: { quantity: 5 } },
      } as unknown as GameState);
