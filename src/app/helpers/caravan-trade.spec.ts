...
describe('caravanTradeRemaining', () => {
  ...
  it('subtracts the count so far from the limit', () => {
    const trade: CaravanTrade = { type: 'sell', value: 10, limit: 5, weight: 1 };
    expect(caravanTradeRemaining(trade, { 0: 2 }, 0)).toBe(3);
  });

  it('floors at 0', () => {
    const trade: CaravanTrade = { type: 'sell', value: 10, limit: 2, weight: 1 };
...
describe('caravanTradeOwnedQuantity', () => {
  ...
  it('resolves owned quantity for an item trade', () => {
    ...
    expect(
      caravanTradeOwnedQuantity({ type: 'buy', value: 1, itemId: 'ore' as ItemId, weight: 1 }),
    ).toBe(7);
  });
  ...
  it('returns 0 for a trade with no target id', () => {
    expect(caravanTradeOwnedQuantity({ type: 'sell', value: 1, weight: 1 })).toBe(0);
  });
});
...
describe('caravanTradeDisplay', () => {
  it('resolves an item trade', () => {
    ...
    expect(
      caravanTradeDisplay({ type: 'sell', value: 1, itemId: item.id, weight: 1 }),
    ).toEqual({
      name: 'Copper Ore',
      description: 'Shiny.',
      sprite: '0001',
...
describe('caravanExecuteTrade', () => {
  ...
  function nodeState(overrides: Partial<CaravanNodeState> = {}): CaravanNodeState {
    return {
      traderId: trader.id,
  ...
  it('returns false for a trade index that is not currently active', () => {
    vi.mocked(caravanState).mockReturnValue(nodeState({ activeTradeIndices: [1] }));

    expect(caravanExecuteTrade(entry, 0)).toBe(false);
  });

  it('returns false once the trade is sold out', () => {
    vi.mocked(caravanState).mockReturnValue(nodeState({ tradeCounts: { 0: 2 } }));
