import { statusEffectTagResistance } from '@helpers/combat-statuseffects';
...
describe('statusEffectTagResistance', () => {
  it('returns 0 when the effect has no tags', () => {
    const combatant = { tagResistance: buildTagResistance({ Stun: 50 }) } as Combatant;
    expect(statusEffectTagResistance(combatant, [])).toBe(0);
  });

  it('returns the matching resistance for a single-tag effect', () => {
    const combatant = { tagResistance: buildTagResistance({ Poison: 6 }) } as Combatant;
