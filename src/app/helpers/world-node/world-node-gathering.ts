...
import type { GatherResult, GatheringContent, MaterialId } from '@interfaces';

// Keyed by (content, level) - level only ever increases (gatherNodeLevelUp), so a level-up is just a miss, never stale.
const resultsAtLevelCache = new WeakMap<
  GatheringContent,
  Map<number, GatherResult[]>
>();
...
export function gatheringResultsAtLevel(
  ...
  const byLevel =
    resultsAtLevelCache.get(gathering) ?? new Map<number, GatherResult[]>();
  const cached = byLevel.get(level);
  if (cached) return cached;

  const results = gathering.gatherResults.filter(
    (result) =>
      result.levelRequirement === undefined ||
      result.levelRequirement === level,
  );
